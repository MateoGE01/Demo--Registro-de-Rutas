...
class Login:
    ...
    def __init__(self, titulo, colorletra, iteracion: bool) -> None:
        ...
        #Será el título
        self.panel_derecha_top = Frame(self.panel_derecha,height = 50, bd=0, relief=SOLID,bg='black')
        self.panel_derecha_top.pack(side="top",fill=X)
        self.title = Label(self.panel_derecha_top, text=self.titulo, font=('Times', 30), fg=self.colorletra,bg='#fcfcfc',pady=50)
        self.title.pack(expand=YES,fill=BOTH)
        #end panel_derecha_top

        #A la derecha del logo y debajo del título "Inicio de Sesion"
        self.panel_derecha_fill = Frame(self.panel_derecha,height = 2,  bd=0, relief=SOLID,bg='#fcfcfc')
        self.panel_derecha_fill.pack(side="bottom",expand=YES,fill=BOTH)

        #Escribir la placa
        self.etiqueta_placa = Label(self.panel_derecha_fill, text="Placa", font=('Times', 14) ,fg="#666a88",bg='#fcfcfc', anchor="w")
        self.etiqueta_placa.pack(fill=X, padx=20,pady=5)
        self.usuario = ttk.Entry(self.panel_derecha_fill, font=('Times', 14))
        self.usuario.pack(fill=X, padx=20,pady=10)

        #Escribir la clave
        self.etiqueta_password = Label(self.panel_derecha_fill, text="Clave", font=('Times', 14),fg="#666a88",bg='#fcfcfc' , anchor="w")
        self.etiqueta_password.pack(fill=X, padx=20,pady=5)
        self.password = ttk.Entry(self.panel_derecha_fill, font=('Times', 14))
        self.password.pack(fill=X, padx=20,pady=10)
        self.password.config(show="*")

        self.inicio = Button(self.panel_derecha_fill,text="Iniciar sesion",font=('Times', 15,BOLD),bg='#3a7ff6', bd=0,fg="#fff", command=self.InicioSesion)
...
class Rutitas:
    ...
    def __init__(self) -> None:
        ...
        #Será el título
        panel_derecha_top = Frame(panel_derecha,height = 50, bd=0, relief=SOLID,bg='black')
        panel_derecha_top.pack(side="top",fill=X)
        title = Label(panel_derecha_top, text="Rutas🛣",font=('Times', 30), fg="#666a88",bg='#fcfcfc',pady=20)
        title.pack(expand=YES,fill=BOTH)
        

        #A la derecha del logo y debajo del título "Inicio de Sesion"
        panel_derecha_fill = Frame(panel_derecha,height = 50,  bd=0, relief=SOLID,bg='#fcfcfc')
        panel_derecha_fill.pack(side="bottom",expand=YES,fill=BOTH)


        inforuta1 = Label(panel_derecha_fill, text="1: Terminal➜Estadio➜Caribe verde➜Uni Atlantico", font=('Times', 14) ,fg="#666a88",bg='#fcfcfc', anchor="w")
        inforuta1.pack(fill=X, padx=20,pady=5)
        inforuta2 = Label(panel_derecha_fill, text="2: Terminal➜Camino murillo➜Puente murillo➜Malecon", font=('Times', 14) ,fg="#666a88",bg='#fcfcfc', anchor="w")
        inforuta2.pack(fill=X, padx=20,pady=5)

        #Botón para ver la ruta
        inicio = Button(panel_derecha_fill,text="Ver ruta",font=('Times', 15,BOLD),bg='#3a7ff6', bd=0,fg="#fff", command=self.EleccionRutas)
        inicio.pack(side="bottom",fill=X, padx=20,pady=20)

        #Pedir al usuario cual ruta desea ver       

        self.usuario = ttk.Entry(panel_derecha_fill, font=('Times', 14))
        self.usuario.pack(side="bottom", fill=X, padx=20,pady=10)
        etiqueta_placa = Label(panel_derecha_fill, text="Ruta deseada", font=('Times', 14) ,fg="#666a88",bg='#fcfcfc', anchor="w")
...
class Estadisticas:
    ...
    def __init__(self, horas_llegada, distancias, hora_ideal, hora_salida, estaciones, i) -> None:
        ...
        self.panel_derecha_top = Frame(self.panel_derecha,height = 50, bd=0, relief=SOLID,bg='black')
        self.panel_derecha_top.pack(side="top",fill=X)
        self.title = Label(self.panel_derecha_top, text="Estadísticas📄",font=('Times', 30), fg="#666a88",bg='#fcfcfc',pady=20)
        self.title.pack(expand=YES,fill=BOTH)
        

        #A la derecha del logo y debajo del título "Inicio de Sesion"
        self.panel_derecha_fill = Frame(self.panel_derecha,height = 50,  bd=0, relief=SOLID,bg='#fcfcfc')
        self.panel_derecha_fill.pack(side="bottom",expand=YES,fill=BOTH)
        ...
        recordatorio = ""
        inforuta1 = Label(self.panel_derecha_fill, text=texto, font=('Times', 14) ,fg="#666a88",bg='#fcfcfc', anchor="w")
        inforuta1.pack(fill=X, padx=20,pady=5)
        if(i == 0):
            ...
            recordatorio = f"          Recuerde que debe estar en: \n        {self.estaciones[2]} a las: {self.horas_llegada[2]}"
        inforuta2 = Label(self.panel_derecha_fill,
        ...
        self.inicio = Button(self.panel_derecha_fill,text="Ver ruta",font=('Times', 15,BOLD),bg='#3a7ff6', bd=0,fg="#fff", command=self.ventana.destroy)
